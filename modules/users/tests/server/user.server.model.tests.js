...
describe('User Model Unit Tests:', function() {
  ...
  describe('Method Save', function() {
    ...
    it('should be able to save without problems', function(done) {
      ...
      var _user1 = User.build(user1);

      _user1.save().then(function(err) {
        should.not.exist(err);
        _user1.destroy().then(function(err) {
          should.not.exist(err);
          done();
        }).catch(function(err) {});
      }).catch(function(err) {});
    ...
    it('should fail to save an existing user again', function(done) {
      var _user1 = User.build(user1);
      var _user2 = User.build(user2);

      _user1.save().then(function() {
        _user2.save().then(function(err) {
    ...
    it('should be able to save 2 different users', function(done) {
      var _user1 = User.build(user1);
      var _user3 = User.build(user3);

      _user1.save().then(function(err) {
        should.not.exist(err);
        _user3.save().then(function(err) {
          should.not.exist(err);
          _user3.destroy().then(function(err) {
            should.not.exist(err);
            _user1.destroy().then(function(err) {
              should.not.exist(err);
              done();
    ...
    it('should not save the passphrase in plain text', function(done) {
      var _user1 = User.build(user1);
      _user1.password = 'Open-Source Full-Stack Solution for SEANJS';
      var passwordBeforeSave = _user1.password;
      _user1.save().then(function(err) {
        should.not.exist(err);
        _user1.password.should.not.equal(passwordBeforeSave);
  ...
  describe("User Password Validation Tests", function() {
    it('should validate when the password strength passes - "P@$$w0rd!!"', function() {
      ...
      _user1.validate().success(function(err) {
        should.not.exist(err);
      }).catch(function(err) {});
    });
    ...
    it('should validate when the passphrase strength passes - "Open-Source Full-Stack Solution For SEANJS Applications"', function() {
      var _user1 = User.build(user1);
      _user1.password = 'Open-Source Full-Stack Solution For SEANJS Applications';
  ...
  describe("User E-mail Validation Tests", function() {
    it('should not allow invalid email address - "123"', function(done) {
      ...
      _user1.save().then(function(err) {
        if (!err) {
          _user1.destroy().then(function(err_remove) {
            should.exist((err) ? null : err);
